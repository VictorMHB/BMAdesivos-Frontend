...
export default function ModalFichaTecnica({ isOpen, onClose, adesivo, itens }) {
  ...
  const tintas = itens.filter((i) => i.insumo.tipoInsumo === "TINTA");
            ...
            {substrato ? (
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                <p className="font-semibold text-blue-900 mb-3">
                  {substrato.insumo.nome}
                </p>

                {/* Aqui foi alterado de grid-cols-2 para grid-cols-3 */}
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <p className="text-xs text-gray-500">Largura</p>
                    <p className="text-sm font-medium text-gray-800">
                      {substrato.insumo.largura != null
                        ? `${substrato.insumo.largura} m`
                        : "—"}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-800">
                      {substrato.insumo.comprimento != null
                        ? `${substrato.insumo.comprimento} m`
                        : "—"}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-800">
                      {substrato.insumo.estoqueAtual != null
                        ? `${substrato.insumo.estoqueAtual} m²`
                        : "—"}
                    </p>
                  </div>
                </div>
            ...
            {tintas.length > 0 ? (
              <div className="space-y-2">
                {tintas.map((item) => (
                  <div
                    key={item.id}
                    className="bg-purple-50 border border-purple-100 rounded-lg p-3 flex items-center justify-between"
                  >
                    <div className="flex items-center gap-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {item.insumo.cor || "—"} ·{" "}
                          {item.insumo.tamanhoEmbalagem
                            ? formatarTamanhoEmbalagem(
                                item.insumo.tamanhoEmbalagem,
                              )
                            : "—"}
                        </p>
                ...
                {resina ? (
                  <div className="bg-yellow-50 border border-yellow-100 rounded-lg p-3 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        Estoque:{" "}
                        {resina.insumo.estoqueAtual != null
                          ? `${resina.insumo.estoqueAtual} kg`
                          : "—"}
                      </p>
                    </div>
                    <div className="text-right">
